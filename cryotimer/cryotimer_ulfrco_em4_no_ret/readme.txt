...
frequency RC oscillator (ULFRCO) as its clock in EM4 without GPIO
retention.
...
Next, EM4 shutdown entry is configured so thsat the ULFRCO is kept active,
and GPIO is retention is disabled (GPIOs immediately reset upon EM4
entry).  The CRYOTIMER is configured to time out after 2K (2048) ULFRCO
clock periods, which is nominally around 2.05 seconds, cause wake-up from
EM4, and request an interrupt in all other energy modes.

With all setup tasks completed, the device enters EM3 and waits for the
...
from EM3.  The interrupt handler clears the CRYOTIMER_IF_PERIOD flag and
...
domains), and exits back to the main().
...
turning off both LEDs.  After 2K (2048) ULFRCO periods, wake-up from EM4
