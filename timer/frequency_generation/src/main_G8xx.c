 ...
 * TIMER module. PD1 (Expansion Header Pin 6) is configured for output compare
 * and toggles PD1 on each overflow event at a set frequency.
 * @version 0.0.1
 ******************************************************************************
 * @section License
...
void initTIMER(void)
{
  ...
  int topValue = CMU_ClockFreqGet(cmuClock_HFPER) / (2*OUT_FREQ * (1 << TIMER0_PRESCALE)) - 1;
