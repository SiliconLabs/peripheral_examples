 ...
 * TIMER module. PD6 (Expansion Header Pin 16) is configured for output compare
 * and toggles PD6 on each overflow event at a set frequency.
 * @version 0.0.1
 ******************************************************************************
 * @section License
...
void initGPIO(void)
{
  ...
  // Configure PD6 (Expansion Header Pin 16) as output
...
void initTIMER(void)
{
  ...
  int topValue = CMU_ClockFreqGet(cmuClock_HFPER) / (2*OUT_FREQ * (1 << TIMER1_PRESCALE)) - 1;
