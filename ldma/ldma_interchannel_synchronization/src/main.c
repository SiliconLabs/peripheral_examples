...
static void gpioPrsSetup(void)
{
  ...
  if ( BSP_GPIO_PB0_PIN > 7 ){
    PRS_SourceSignalSet(GPIO_PRS_CHANNEL, PRS_CH_CTRL_SOURCESEL_GPIOH, BSP_GPIO_PB0_PIN - 8, prsEdgePos);
  }
  else {
    PRS_SourceSignalSet(GPIO_PRS_CHANNEL, PRS_CH_CTRL_SOURCESEL_GPIOL, BSP_GPIO_PB0_PIN, prsEdgePos);
  }
  if ( BSP_GPIO_PB1_PIN > 7 ){
    PRS_SourceSignalSet(GPIO_PRS_CHANNEL+1, PRS_CH_CTRL_SOURCESEL_GPIOH, BSP_GPIO_PB1_PIN - 8, prsEdgePos);
  }
  else {
