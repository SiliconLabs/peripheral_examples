...
static void setupPcnt(void)
{
  ...
  pcntInit.mode     = pcntModeExtQuad;          // External quadratur emode
  pcntInit.top      = 5;
  pcntInit.s1CntDir = false;                    // S1 does not affect counter direction, default count up
  pcntInit.s0PRS    = pcntPRSCh0;               // PCNT0 S0PRS linked to PRSCH0
  pcntInit.s1PRS    = pcntPRSCh1;               // PCNT0 S1PRS linked to PRSCH1
...
/***************************************************************************//**
 * @brief PRS setup
 *        This function sets up GPIO PRS pin 8 and pin 11 which links BNT0
 *        and BTN1 to PCNT0 PRS0 and PCNT0 PRS1        
 ******************************************************************************/        
static void setupPrs(void)
{
  ...
  /* Set up GPIO PRS pin 11 */
  PRS_SourceAsyncSignalSet(1, PRS_CH_CTRL_SOURCESEL_GPIOH, PRS_CH_CTRL_SIGSEL_GPIOPIN11);
}
...
static void setupGpio(void)
{
  ...
  GPIO_ExtIntConfig(BSP_GPIO_PB1_PORT, BSP_GPIO_PB1_PIN, 11, false, false, false);
}
