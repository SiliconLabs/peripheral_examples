...
void initDma(void)
{
  ...
  DMA_ActivatePingPong(channelNum,
                       isUseBurst,
                       (void *) &TIMER1->CC[0].CCVB, // Primary destination address to transfer to
                       (void *) &buffer,             // Primary source address to transfer from
                       BUFFER_SIZE - 1,              // Primary number of DMA transfers minus 1
                       (void *) &TIMER1->CC[0].CCVB, // Alternate destination address to transfer to
