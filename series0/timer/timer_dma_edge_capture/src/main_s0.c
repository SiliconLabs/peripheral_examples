 ...
 * captured by TIMER1 CC0 are transferred to a fixed length buffer by the DMA.
...
void initGpio(void)
{
  ...
  // Set TIM1_CC0 #4 GPIO Pin (PD6) as Input
...
void initDma(void)
{
  ...
  DMA_ActivateBasic(channelNum,
                    ...
                    (void *) &(TIMER1->CC[0].CCV), // Source address to transfer from
