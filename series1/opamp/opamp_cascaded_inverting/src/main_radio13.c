...
int main(void)
{
  ...
  init0.posSel   = opaPosSelAPORT1XCH8;  // Choose opamp positive input to come from PC8
  ...
  init1.posSel = opaPosSelAPORT2XCH9;    // Choose opamp positive input to come from PC9

  // Configure OPA2
  OPAMP_Init_TypeDef init2 = OPA_INIT_CASCADED_INVERTING_OPA2;
  init2.resSel  = RESISTOR_SELECT;      // Choose the resistor ladder ratio
  init2.posSel  = opaPosSelAPORT3XCH8; // Choose opamp positive input to come from PA0
  init2.outMode = opaOutModeAPORT3YCH9; // Route opamp output to PA1
