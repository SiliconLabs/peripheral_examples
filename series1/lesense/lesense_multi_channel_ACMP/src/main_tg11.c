...
void setupACMP(void)
{
  ...
  ACMP1 -> INPUTSEL = (ACMP1 -> INPUTSEL & _ACMP_INPUTSEL_NEGSEL_MASK)
...
void initGPIO(void)
{
  ...
  // LESENSE channel 0-3 can be mapped to either ACMP0 port or
  // ACMP1 port when only one of the ACMP is used
...
void setupLESENSE(void)
{
  ...
  initLesense.perCtrl.acmp0Mode = lesenseACMPModeDisable;
