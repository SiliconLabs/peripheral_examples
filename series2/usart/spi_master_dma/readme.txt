...
problematic depending on the slave device's timing requirements.  The
slave mode examples that can be run on another WSTK to verify operation
...
1. This example requires two Starter Kits (STKs) for any two EFM32 or
   EFR32 devices.  Build the spi_slave_dma example and download it to
   the first board.
...
4. Before running the example, set a breakpoint on the rx_done = false
   ...
   received data from the slave.  This example transfers data continuously
