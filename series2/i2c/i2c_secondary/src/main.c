/***************************************************************************//**
 * @file main.c
 * @brief This project demonstrates the secondary configuration of the EFx32xG2x
 * I2C peripheral. Two EFx32xG2x devices are connected together, one running the
 * main project, the other running the secondary project. The main reads the
 ...
 * handle I2C transmissions. Secondary toggles LED0 on during I2C transaction
 * and off when complete. Secondary will set LED1 if an I2C transmission error
 * is encountered.
 *******************************************************************************
 * # License
...
void initI2C(void)
{
  ...
  // Configure to be addressable as secondary
  ...
  // Set up to enable secondary mode
...
void I2C0_IRQHandler(void)
{
  ...
  if (pending & (I2C_IF_BUSERR | I2C_IF_ARBLOST)) {
    ...
    if (pending & I2C_IF_ADDR) {
      ...
      if (rxData & 0x1) { // read bit set
        if (i2c_BufferIndex < I2C_BUFFER_SIZE) {
          ...
          // Invalid buffer index; transfer data as if secondary non-responsive
          I2C0->TXDATA     = 0xFF;
        }
      } else {
    ...
    if (pending & I2C_IF_ACK) {
      /******************************************************/
      /* Main ACK'ed, so requesting more data               */
      /******************************************************/
      if (i2c_BufferIndex < I2C_BUFFER_SIZE) {
        // Transfer data
        I2C0->TXDATA = i2c_Buffer[i2c_BufferIndex++];
