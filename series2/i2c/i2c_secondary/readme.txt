...
This project demonstrates the secondary configuration of the EFx32xG2x I2C
peripheral. Two EFx32xG2x devices are connected together, one running the main
project, the other running the secondary project. The main reads the
secondary's current buffer values, increments each value, and writes the new
values back to the secondary device. The main then reads back the secondary
...
handle I2C transmissions. Secondary toggles LED0 on during I2C transaction and
off when complete. Secondary will set LED1 if an I2C transmission error is
encountered.
...
compare the between the current consumption value described in the datasheet
and the current consumption measured with Simplicity Energy Profiler. These
settings do not affect the functionality of this example and can be removed.
Note that the board's external SPI flash is also powered down as the SPI
flash's current is also measured by the Energy Profiler tool.
...
4.  Build both the main and secondary projects and download to two Starter Kits
5.  For secondary kit, in the drop-down menu, select "Profile As Simplicity
...
8.  Toggle push button PB0 on the main kit
9.  Observe the current spike as the secondary kit wakes to EM0/1 to handle the
    I2C transaction and then returns to EM2 consumption with each button press
    of PB0 on the main kit.
