/***************************************************************************//**
 * @file main.c
 * @brief This project demonstrates the main configuration of the EFx32xG2x I2C
 * peripheral. Two EFx32xG2x devices are connected together, one running the
 * main project, the other running the secondary project. The main starts up
 ...
 * reads the secondary device's current buffer values, increments each value by
 * 1, and writes the new values back to the secondary device. The main then
 * reads back the secondary values again and verifies the new values match what
 ...
 * with every PB0 button press and toggling LED0 with each successful
 * iteration. If there is an I2C transmission error, or if the verification
 * step of the I2C test fails, LED1 is turned on and the main sits and remains
 * in an infinite while loop. Connecting to the device via debugger while in
 * the infinite loop, the I2C error code can be retrieved.
 *******************************************************************************
 * # License
...
// Defines
#define I2C_SECONDARY_ADDRESS               0xE2
...
/***************************************************************************//**
 * @brief I2C read numBytes from secondary device starting at target address
 ******************************************************************************/
void I2C_MainRead(uint16_t secondaryAddress, uint8_t targetAddress, uint8_t *rxBuff, uint8_t numBytes)
{
  // Transfer structure
  ...
  i2cTransfer.addr          = secondaryAddress;
...
/***************************************************************************//**
 * @brief I2C write numBytes to secondary device starting at target address
 ******************************************************************************/
void I2C_MainWrite(uint16_t secondaryAddress, uint8_t targetAddress, uint8_t *txBuff, uint8_t numBytes)
{
  // Transfer structure
  I2C_TransferSeq_TypeDef i2cTransfer;
  I2C_TransferReturn_TypeDef result;
...
bool testI2C(void)
{
  ...
  // Initial read of bytes from secondary
  I2C_MainRead(I2C_SECONDARY_ADDRESS, 0, i2c_rxBuffer, I2C_RXBUFFER_SIZE);

  // Increment received values and prepare to write back to secondary
  for (i = 0; i < I2C_RXBUFFER_SIZE; i++) {
    i2c_txBuffer[i] = i2c_rxBuffer[i] + 1;
  }

  // Block write new values to secondary
  I2C_MainWrite(I2C_SECONDARY_ADDRESS, 0, i2c_txBuffer, I2C_TXBUFFER_SIZE);

  // Block read from secondary
  I2C_MainRead(I2C_SECONDARY_ADDRESS, 0, i2c_rxBuffer, I2C_RXBUFFER_SIZE);
