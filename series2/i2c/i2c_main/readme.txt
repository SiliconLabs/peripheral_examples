...
This project demonstrates the main configuration of the EFx32xG2x I2C
peripheral. Two EFx32xG2x devices are connected together, one running the main
project, the other running the secondary project. The main starts up and enters
a while loop waiting for a button press on push button 0. When push button 0 is
pressed, the program performs an I2C test. This routine reads the secondary
...
loop, waiting again for user input through push button 0. This project runs in
a continuous loop, re-running the I2C test with every PB0 button press and
...
the main sits and remains in an infinite while loop. Connecting to the device
...
5.  On main kit, project will compile, load and start-up, proceeding to the
    main loop 
6.  Press PB0 on the main kit to trigger an I2C test (read secondary buffer,
    increment, write, read, verify)
7.  Observe LED0 toggle with a successful I2C test
8.  Suspend (pause) the main kit via debugger session
9.  Observe that received values received from the secondary (rxBuffer) match
    transmitted values sent by main (txBuffer) by viewing "rxBuffer" and
