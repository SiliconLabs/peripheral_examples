...
result in a global variable. Varying from the iadc_scan_letimer_interrupt
example, the IADC operates on a single external input, and since IADC port/pin
selection does not switch for single conversions, the input selection is not
limited to ports A/B while operating in EM2. GPIO are toggled with LETIMER
underflow and IADC conversion complete. LETIMER GPIO is toggled through LETIMER 
output, and thus must be routed to port A/B pins when operating in EM2. IADC
