...
// Number of wtimer overflows since last edge capture;
...
void initWTIMER(void)
{
  ...
  // Enable WTIMER0 interrupt flags
...
int main(void)
{
  ...
  {
    // Wait for flags
    while(!(TIMER_IntGetEnabled(WTIMER0) & (TIMER_IF_OF | TIMER_IF_CC0)));
    
    // Get pending flags and clear
    irq_flags = TIMER_IntGet(WTIMER0);
    TIMER_IntClear(WTIMER0, TIMER_IF_CC0 | TIMER_IF_OF);

    // Read the last captured value from the CC register
    uint32_t current_edge = TIMER_CaptureGet(WTIMER0, 0);

    // Check if wtimer overflow occurred between edges
