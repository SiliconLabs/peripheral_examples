...
void initUSART1 (void)
{
  ...
  // Configure GPIO mode
  GPIO_PinModeSet(gpioPortC, 15, gpioModeInput, 1);   // US1_CLK is input
  GPIO_PinModeSet(gpioPortC, 14, gpioModeInput, 1);   // US1_CS is input
  GPIO_PinModeSet(gpioPortD, 7, gpioModeInput, 1);    // US1_TX (MOSI) is input
  GPIO_PinModeSet(gpioPortD, 6, gpioModePushPull, 1); // US1_RX (MISO) is push pull
  ...
  config.enable    = false;           // Make sure to keep USART disabled until it's all set up
