...
{
  volatile uint32_t clearFlag = TIMER0->CC[0].CCV;  // Reading this clears the interrupt flag

  numTransfers++;
  uint8_t *newStart = &TxBuffer[numTransfers];
  if(numTransfers < 10)
  {
    DMA_ActivateBasic(channel, primary, false, (void*) &USART0->TXDATA, (void*) newStart, 0);
  }
}
...
void initUSART0 (void)
{
  ...
  // Configure GPIO mode
  GPIO_PinModeSet(gpioPortA, 12, gpioModePushPull, 0); // US1_CLK is push pull
  GPIO_PinModeSet(gpioPortC, 8, gpioModePushPull, 1);  // US1_CS is push pull
  GPIO_PinModeSet(gpioPortC, 11, gpioModePushPull, 1); // US1_TX (MOSI) is push pull
  GPIO_PinModeSet(gpioPortC, 10, gpioModeInput, 1);    // US1_RX (MISO) is input
...
void initTransferDma(void)
{
  ...
  descriptorConfigTX.srcInc  = dmaDataInc1;     // Source moves one spot in the buffer every transmit
